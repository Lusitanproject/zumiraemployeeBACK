...
import { GetAssessmentResultUserFiltersController } from "../../controllers/admin/assessments/GetAssessmentResultUserFiltersController";
import { SearchAssessmentResultsController } from "../../controllers/admin/assessments/SearchAssessmentResultsController";
...
/**
 * @swagger
 * /admin/assessments/{id}/results/user-filters:
 *   get:
 *     summary: "[Admin] Filtros de usuários com resultados da avaliação"
 *     description: >
 *       Retorna os valores distintos das colunas demográficas dos usuários que possuem resultados
 *       para a avaliação informada. Útil para popular os filtros na interface antes de chamar
 ...
 *           type: string
 *           format: cuid
 *         description: Filtrar apenas usuários da empresa informada (opcional)
 *       - in: query
 *         name: columns
 *         required: true
 *         style: form
 ...
 *         $ref: '#/components/responses/Unauthorized'
 */
adminAssessmentRouter.get("/:id/results/user-filters", isAuthenticated, new GetAssessmentResultUserFiltersController().handle);

/**
 * @swagger
 * /admin/assessments/{id}/results:
 *   get:
 *     summary: "[Admin] Buscar resultados paginados de uma avaliação"
 *     description: >
 *       Retorna os resultados (último por usuário) de uma avaliação específica com suporte a
 *       paginação e filtros demográficos dos respondentes. `companyId` é opcional; sem ele,
 *       retorna resultados de todas as empresas.
 *     tags: [Admin - Assessments]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *         schema:
 *           type: string
 *         description: Busca parcial (case-insensitive) no campo `profile` da faixa de risco
 *       - in: query
 *         name: page
 *         schema:
 *           type: integer
 ...
 *         $ref: '#/components/responses/Unauthorized'
 */
adminAssessmentRouter.get("/:id/results", isAuthenticated, new SearchAssessmentResultsController().handle);
